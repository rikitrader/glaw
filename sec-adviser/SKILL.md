...
**complements** the build-side seats — `fund-regulatory-council` and `pe-vc-counsel` own
...
Quantify the fee/expense/valuation impact (route the math to `financial-forensics`). Grade
...
### Step 5 — Route
- Fee/expense/valuation and NAV math → `financial-forensics` + `/glaw-audit-assurance`.
- Formation/compliance doctrine and filings → `fund-regulatory-council` + `pe-vc-counsel`.
- Digital-asset funds/tokens → `tokenization-compliance` + `/glaw-fincen-crypto`.
