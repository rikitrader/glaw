...
2. **Plead every viable count (Build-the-whole-file).** Route claim development to
   **`elite-corporate-counsel`** (fraud-on-court, FUFTA, veil-piercing, civil theft,
   MCA/usury) and **`federal-trial-counsel`** (federal claims, RICO, removal/venue).
...
4. **Develop the facts.** Where elements lack proof, route to
   **`forensic-case-investigator`** (connect the dots, follow the money, who controls
   the entities) and **`financial-forensics`** (reconstruct the numbers from records).
...
5. **Damages theory.** Quantify each measure: actual/compensatory, statutory (note
   ...
   hooks, pre/post-judgment interest. Route quantification to **`financial-forensics`**
   or **`company-valuation`** where a number must be modeled.
...
2. **Candidate entity strategies (with tradeoffs).** Route to
   **`institutional-finance`** and **`pe-vc-counsel`**. Lay out 2–3 viable paths,
...
3. **Tax posture (high level).** Route to **`tax-strategy`**: pass-through vs.
...
4. **Capital plan.** How it gets funded: founder/bootstrapped, SAFE/priced round,
   ...
   framing to **`pe-vc-counsel`** / **`institutional-finance`**.
5. **Exit.** Acquisition, IPO, secondary, fund wind-down/distribution waterfall.
   Route a valuation anchor to **`company-valuation`** where the thesis turns on a number.
