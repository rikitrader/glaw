...
event studies), and market microstructure. It routes the numbers to `financial-forensics`
and `/glaw-valuation-409a` / `company-valuation`, the event-study and loss-causation
...
- **Forensic accounting** — reconstruct from primary records with a documented audit
  trail; route the reconstruction to `financial-forensics` so every figure is sourced.
...
section: numbers/reconstruction → `financial-forensics`; valuation/damages →
`company-valuation` + `/glaw-valuation-409a`; event study and loss causation →
