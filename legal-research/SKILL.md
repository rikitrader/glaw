...
- **Tax** authority/elections → re-cite via `tax-strategy` / `tax-compliance`, then verify.
- **Securities** authority (Reg D, Advisers Act, etc.) → `pe-vc-counsel` / `fund-regulatory-council` re-cites, we verify.
- **Litigation** holdings and procedural rules → `elite-corporate-counsel` / `federal-trial-counsel` re-cite; we confirm reporter, holding, and good-law status.
