...
`forensic-case-investigator`, and the numbers to `financial-forensics`.
...
Develop the facts behind the tip with `forensic-case-investigator`; quantify sanctions /
ill-gotten gains for the award threshold with `financial-forensics`. Send the
...
4. **Award-positioning** — quantify the >$1M sanction theory with `financial-forensics`; map
