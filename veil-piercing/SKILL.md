...
the shield**. It complements `elite-corporate-counsel`, which owns the substantive
...
`elite-corporate-counsel` and every citation passes through `/glaw-legal-research`.
...
(route to `elite-corporate-counsel` / `/glaw-restructuring`).
...
- **Defend matrix** — for each factor, the facts and documents that **defeat** piercing.
  ...
  the cure to `corporate-counsel` (formation/governance) so the formalities get papered
...
and hand the legal call to `elite-corporate-counsel`.

### Step 5 — Route and verify
- **Doctrine / standard / the legal call** → `elite-corporate-counsel`.
...
- **Fraudulent-transfer / FUFTA parallel** → `elite-corporate-counsel` / `/glaw-restructuring`.
- **Shield-gap cure documents** → `corporate-counsel`.
- **Forensic tracing of commingled funds** → `/glaw-accounting` (`financial-forensics`).
...
doctrine and the controlling standard belong to `elite-corporate-counsel`; every
