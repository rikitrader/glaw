...
1. **Charter documents.** Certificate of incorporation (Delaware) or articles of
   ...
   structure. Route to **`corporate-counsel`**.
2. **Governance.** Bylaws (corp) or operating agreement (LLC) / LP agreement (fund),
   ...
   **`corporate-counsel`**; fund LPAs to **`pe-vc-counsel`**.
...
4. **Founder package.** Founder agreements, restricted-stock purchase agreements with
   ...
   Route to **`corporate-counsel`**; specialized IP assignment to **`/glaw-ip-counsel`**.
5. **Equity docs.** SAFE / convertible note, stock purchase agreement, and the
   ...
   to **`pe-vc-counsel`** / **`corporate-counsel`**.
6. **Offering package (if raising).** PPM, subscription agreement, and the **Form D**
   ...
   **`fund-regulatory-council`** (Form D / Blue Sky / PPM supplements) and
   **`pe-vc-counsel`** (PPM/sub-agreement). For tokenized offerings, route to
   **`tokenization-compliance`**. Tax disclosure sections route to **`tax-compliance`**.
7. **Render.** Produce final formatted documents via **`document-generate`**,
   **`docx`**, or **`make-pdf`** — each carrying the UPL footer.
...
1. **Complaint or answer.** Complaint: caption, jurisdiction+venue allegations,
   ...
   Route to **`federal-trial-counsel`** and **`elite-corporate-counsel`**.
...
3. **Discovery requests.** Interrogatories, requests for production, requests for
   ...
   strategy/structure. Route to **`federal-trial-counsel`**.
...
5. **Render.** Format via **`document-generate`** / **`docx`** / **`make-pdf`**, UPL
   footer on each.
