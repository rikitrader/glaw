...
const SKILLS_HOME = process.env.GLAW_SKILLS_HOME || `${process.env.CODEX_HOME || `${process.env.HOME}/.codex`}/skills`
const GLAW_ROOT = process.env.GLAW || process.env.GLAW_SKILL_ROOT || `${SKILLS_HOME}/glaw`
const GLAW_83B_ROOT = process.env.GLAW_83B_ROOT || `${GLAW_ROOT}/seats/glaw-83b-election`
const ROSTER = `${GLAW_ROOT}/lib/firm-roster.md`
const MAX_ROUNDS = 2
...
const guard = await agent(
  ...
  `python3 ${GLAW_ROOT}/bin/matter-ops/facts_validate.py ${process.env.GLAW_MATTER || '<matter-slug>'}`,
...
const comments = await agent(
  `Run: python3 ${GLAW_83B_ROOT}/bin/review_comments.py ${FOLDER}. ` +
...
const priorLearnings = await agent(
  ...
  `(1) run via Bash and include verbatim: python3 ${GLAW_ROOT}/bin/glaw-learnings preflight; ` +
...
const learned = await agent(
  ...
  `python3 ${GLAW_ROOT}/bin/glaw-learnings add ` +
  ...
  `meta-rules from the accumulated ledger: python3 ${GLAW_ROOT}/bin/glaw-reflect --apply. ` +
  `Return the list of learnings recorded + the new ledger stat ` +
  `(run: python3 ${GLAW_ROOT}/bin/glaw-learnings stats).`,
...
if (allClear){
  ...
  finalize = await agent(
    ...
    `bash ${GLAW_ROOT}/bin/matter-ops/assemble_package.sh ` +
    ...
    `python3 ${GLAW_ROOT}/bin/matter-ops/publish_drafts_to_drive.py ${FOLDER} ` +
