...
| `glaw-headless` | `--goal "<objective>" [--matter SLUG] [--json]`; read-only spawned/orchestrator report with matter, stage, track, loop decision, open gates, next owner/command, final-packet summary, compliance manifest, government-adversary manifest, accounting-control manifest/failures for bank reconciliation and tax tie-out routing, decisions, artifacts, and human-seal boundary. `glaw --headless --goal ...` delegates here |
