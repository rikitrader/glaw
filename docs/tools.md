...
| `glaw-citation-gate` | `record --id <id> --proposition ... --authority ... --status verified --support-summary ... --source-url https://... --reviewer legal-research` · failed rows require `--defect-type incorrect\|misgrounded\|ungrounded\|incomplete` · `status` · `complete`; logs `citations_verified` only when every latest citation row is verified with proposition, authority, support summary, the legal-research reviewer, and a valid HTTP(S) source URL |
