...
| `glaw-adversarial` | `record --profile auto --lens <government/regulatory lens> --decision survive\|fix\|strike` · `status` · `complete`; `survive` requires source-backed `--evidence` plus a source-cited `--attack` challenge summary, `fix/strike` attacks must cite `SRC-####`, `fix` requires `--cure`, and `adversarial_done` logs only when every required RED-team lens survives |
