...
| `glaw-chief-decision` | record the Chief's PROCEED / WITH-FIXES / WITH-CONDITIONS sign-off → matter timeline + decision card; `--approve-final` requires a ready `final_packet.json` and binds approval to that packet's `generated_at` plus SHA-256 digest |
