...
| `glaw-sandbox` | `list [--json]` · `run [--scenario all\|NAME] [--json]`; source-only isolated simulation runner. It creates temporary `$GLAW_HOME` fixtures and proves fail-closed behavior for conscience human-only acts, Oversight Board kill-switch, daemon deadline routing, jurisdiction pack failures, reviewer profile-map consistency, government/regulatory/litigation adversary coverage, and final-packet government-adversary failures routing back to `/glaw-adversarial` |
