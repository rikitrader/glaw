...
to `tax-strategy`, the fund mechanics to `pe-vc-counsel`, and the sanctions analysis
...
Hand all quantification and the elections to `tax-strategy`.
...
`tax-strategy` to prepare. Where a **foreign fund** is involved, frame the vehicle
choice and **AIFMD** marketing/registration or reverse-solicitation posture, then
hand the fund mechanics, LPA/PPM, and Reg S/securities analysis to `pe-vc-counsel`
(and `fund-regulatory-council` for filings).
...
- **Tax computation, treaty/anti-deferral math, U.S. reporting forms** → `tax-strategy`.
- **Fund mechanics, LPA/PPM, Reg S, AIFMD detail** → `pe-vc-counsel` / `fund-regulatory-council`.
