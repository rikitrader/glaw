...
one self-contained, branded HTML document — print-to-PDF ready. Optionally pushes the
trial balance to a Google Sheet (reusing the engine's Sheets path) when ~/.gcp/token.json
is present. Every number comes from the posted ledger.
"""
...
def push_gsheet(book: str, title: str) -> str:
    """Best-effort push of the trial balance to a Google Sheet (needs ~/.gcp/token.json)."""
    token = Path.home() / ".gcp" / "token.json"
    if not token.exists():
        return "skipped — ~/.gcp/token.json not found (run the Google OAuth setup first)"
    try:
        ...
        values = [["Account", "Debit", "Credit"]] + [
            [r["account"], str(r["debit"]), str(r["credit"])] for r in tb["rows"]]
        ss = svc.spreadsheets().create(body={"properties": {"title": title}}).execute()
        sid = ss["spreadsheetId"]
        svc.spreadsheets().values().update(
            spreadsheetId=sid, range="A1", valueInputOption="RAW", body={"values": values}).execute()
        return f"https://docs.google.com/spreadsheets/d/{sid}"
    except Exception as e:                          # pragma: no cover (network path)
        return f"gsheet push failed: {e}"
...
def main() -> int:
    ...
    ap.add_argument("--gsheet", action="store_true", help="also push the trial balance to a Google Sheet")
    a = ap.parse_args()
