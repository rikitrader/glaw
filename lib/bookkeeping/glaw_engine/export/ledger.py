...
Usage::
    ...
    from glaw_engine.export import to_hledger, to_beancount
...
# GLAW patch (vendored): account roots that must NOT be re-prefixed with
# "Expenses:". An AccountMapper that already emits a full path like
# "Income:Salary" or "Assets:Bank:Savings" should pass through verbatim so
# income and transfers don't get mis-booked as expenses.
_LEDGER_ROOTS = (
    "Expenses:", "Income:", "Revenue:", "Assets:",
    "Liabilities:", "Equity:",
)
...
def _resolve_contra(
    ...
    If the category already starts with a recognized ledger root
    (Income:, Assets:, ...), it is used verbatim. Otherwise it is
    treated as a short bucket and nested under ``Expenses:``.
    """
    category = tx.category
    if category:
        safe = category.replace(" ", ":").replace("/", ":")
        if safe.startswith(_LEDGER_ROOTS):
            return safe
