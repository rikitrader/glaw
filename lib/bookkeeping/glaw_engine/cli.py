...
class BankStatementCLI:
    ...
    def run_review(
        ...
        :class:`~glaw_engine.hybrid.IngestResult` via its
        ...
        * ``q`` quit early without writing further changes
        ...
        """
        try:
            from glaw_engine.hybrid import (
                IngestResult,
                VerificationStatus,
            )
        except ImportError as exc:
    ...
    def _edit_review_row(self, tx: Transaction) -> Transaction:
        ...
        from glaw_engine.transaction_models import (
...
def main() -> None:
    ...
    users can run::
        ...
        glaw_engine --type ingest --input statement.pdf

    instead of::

        python -m glaw_engine.cli --type ingest --input statement.pdf
    """
