...
the actual estate/gift/GST computation and the planning strategy to `tax-strategy`.
...
`tax-strategy`; route any non-filer/back-tax cleanup to `tax-compliance`.
...
and the litigation exposure to `elite-corporate-counsel` (FUFTA) — do not paper a
...
- **Estate/gift/GST computation + exemption strategy** → `tax-strategy`; controversy/back-tax → `tax-compliance`.
- **Entity layering that holds the protected assets** → `/glaw-structure`.
- **Fraudulent-transfer exposure** → `/glaw-restructuring` (bankruptcy §548) and `elite-corporate-counsel` (FUFTA litigation).
- **Buy-sell valuation** → `company-valuation` / `institutional-finance`.
...
- A transfer-tax **flag memo** (exposure identified, computation deferred to `tax-strategy`).
