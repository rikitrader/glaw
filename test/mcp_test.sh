...
"$MCP" call glaw_execute --arguments '{"tool":"glaw","args":["version"]}' --json > "$TMP/call-ok.json"; rc=$?
python3 - "$TMP/call-ok.json" <<'PY'
import json, sys
outer = json.load(open(sys.argv[1], encoding="utf-8"))
inner = json.loads(outer["content"][0]["text"])
ok = outer["isError"] is False and inner["status"] == "pass" and inner["pre_guard"]["status"] == "pass"
...
msgs = [
    ...
    {"jsonrpc": "2.0", "id": 3, "method": "tools/call", "params": {"name": "glaw_execute", "arguments": {"tool": "glaw", "args": ["version"]}}},
]
