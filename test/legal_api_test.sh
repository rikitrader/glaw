...
const store = new Map();
const env = {
  LEGAL_TENANT_TOKENS: JSON.stringify({ "tenant-a": "tenant-a-secret", "tenant-b": "tenant-b-secret" }),
  INTAKE_KV: {
...
const call = (path, method, body, token = "tenant-a-secret") => mod.default.fetch(new Request(`https://glaw.test${path}`, {
...
response = await call(`/legal/requests/${id}/governor`, "GET", null, "tenant-b-secret");
if (response.status !== 404) throw new Error("tenant B must not read tenant A's request");
...
for (const key of auditKeys) {
  if (JSON.parse(store.get(key).value).tenant_id !== "tenant-a") throw new Error("audit event lost tenant binding");
}
