...
JSON
python3 - "$ROOT/bin/glaw-fpa-engine" "$TMP/request.json" <<'PY'
import json
import subprocess
import sys

engine, request = sys.argv[1:]
...
if completed.returncode != 0:
    raise SystemExit(f"validation command failed ({completed.returncode}): {completed.stderr}")
