...
reconstructed from raw records (that's `financial-forensics`) and not forward-modeled
(that's `institutional-finance`), but made *audit-ready*: controls in place, statements
...
- **Reconstructing** statements from raw bank/card/processor records → `financial-forensics`. This seat assures what exists; it does not rebuild the books.
- **Valuation** of the business → `company-valuation` (or `institutional-finance`).
- **Tax provision / deferred taxes / entity tax posture** → `tax-strategy`.
- **Industry job-cost detail** (WIP, percentage-of-completion, retainage) → `roofer-accounting`; this seat assures the *presentation* of it.
- **Forward 3-statement / LBO / fund models** → `institutional-finance`.
