...
document: US Letter, 1in margins, Times New Roman 12pt justified body, bold UPPER section headings, bold run-in
subsection leads, borderless-but-bordered tables, small italic disclaimer notes. Brackets preserved (blank fillable).
...
def setfont(run, size=12, bold=False, italic=False, color=(0,0,0)):
...
def add_inline(p, text, base_bold=False, size=12):
    # split on **bold**
...
def convert(src, out):
    ...
    while i < len(md):
        ...
        if m:
            ...
            if lvl == 1:
                p.alignment = WD_ALIGN_PARAGRAPH.CENTER; setfont(p.add_run(txt), 13, bold=True)
            else:
                r = p.add_run(txt.upper() if lvl == 2 else txt); setfont(r, 12, bold=True); r.font.underline = (lvl<=3)
            i += 1; continue
        if ln.startswith(">"):
            buf = []
