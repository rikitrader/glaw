...
- ✅ **P2-G3 · Cross-matter memory**: `glaw-learnings` is now a queryable source-linked
  ...
  `$GLAW_HOME` support, reflected knowledge rules written back to the same ledger,
  operator-facing `eval` scoring for expected top-hit retrieval, and regression coverage
  proving accounting-tax, litigation, SEC reporting, tax, and legal-research tracks can rank
  different lessons for overlapping queries.
...
- ✅ **P2-G12 · Oversight Board policy packs**: `/glaw-oversight-board` now ships
  ...
  escalation triggers, human-seal boundaries, prohibited autonomous acts, decision rules,
  and source-evidence requirements are present.
...
- ✅ **P4-G1b · Authority-source catalog for Fortune 500 workflows**:
  `lib/glaw_authority_sources.py` now exposes a code-reviewed source catalog and
  `glaw-citation-corpus sources [--profile ...]` lists official lanes for legal research,
  tax, accounting-tax, SEC-reporting, and public-law work. Captured corpus rows carry
...
- ✅ **P4-G3b · File-stage groundedness recomputation**: `glaw-gate check file`
  now recomputes groundedness from current citations/corpus and refuses a stale or
  mismatched `groundedness.json` even when `final_packet.json` has been rebuilt with
  matching artifact hashes. `gate_test.sh` corrupts the artifact, updates packet and
  approval hashes, and proves the gate still blocks.
...
- ✅ **P4-G4c · California enterprise authority pack**:
  `jurisdiction/packs/us-california-enterprise.json` adds source-backed California entity,
  franchise-tax, privacy/data-broker, labor-standards, and workplace-safety lanes for
  high-risk enterprise matters, and `jurisdiction_pack_test.sh` requires it to validate
  with zero review warnings.
