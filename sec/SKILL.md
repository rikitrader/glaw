...
Optional engines: **`sec-enforcement-swarm`** for multi-agent enforcement sweeps;
...
2. **ANALYSIS (parallel)** — deploy in one batch, each returns sourced findings keyed to the matrix:
   ...
   - `financial-forensics` + `/glaw-accounting` — forensic accounting (rev-rec, restatement, disgorgement math).
