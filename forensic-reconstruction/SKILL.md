...
client extends as each issue is cured). Exit code is non-zero until AUDIT-READY. Detailed period
reports come from `glaw-forensic-reports` (monthly/yearly P&L + full transaction trace). The deeper
LLM adversarial debate (`/glaw-adversarial` → `/glaw-chief-decision`) layers on top for judgment
calls. Conflicts cleared · citations verified · UPL footer also apply to every deliverable.
