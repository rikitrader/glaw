...
`/glaw-court-records`, `/glaw-bureau`, `forensic-case-investigator`,
`financial-forensics`) the way the lead attorney later commands the detection cell.
...
`forensic-case-investigator` and `financial-forensics`. Stand up the wider field/OSINT
