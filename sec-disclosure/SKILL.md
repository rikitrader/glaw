...
### Step 6 — Route
- Numbers, restatement, and accounting-fraud mechanics → `financial-forensics` +
  `/glaw-audit-assurance`.
- Disclosure tied to an offering's registration/exemption → `pe-vc-counsel` +
  `fund-regulatory-council`; digital-asset offerings → `tokenization-compliance`.
